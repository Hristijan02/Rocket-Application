...
namespace RocketApplication
{
    public class SQLiteDataAccess
    {
        ...
        public static void SaveRocket(RocketModel model)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionsString()))
            {
                cnn.Execute("insert into Rocket (RocketName, LaunchLocation, LaunchTime, Id, ImageURL, LaunchStatus) " +
                    "values (@RocketName, @LaunchLocation, @LaunchTime, @Id, @ImageURL, @LaunchStatus)", model);
            }
        }
        ...
        public static void UpdateRocket(RocketModel model)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionsString()))
            {
                cnn.Execute("UPDATE Rocket SET RocketName = @RocketName, LaunchLocation = @LaunchLocation, LaunchTime = @LaunchTime" +
                    ", ImageURL = @ImageURL, LaunchStatus = @LaunchStatus WHERE Id = @Id", model);
            }
        }
